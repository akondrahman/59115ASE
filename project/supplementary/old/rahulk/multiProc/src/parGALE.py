...
def gale0(model=DTLZ2(n_dec=17,n_obj=2), new=[], pop=int(1e4)):
  """
  ...
  def mutate(lst,good,g=0.15):
    new=[]
    #print("len of lst inside mutatue ...", len(lst))
    for l in lst:
        new.append([a+(b-a)*g for a,b in zip(l,good)])
        #print("len of new inside mutatue ...", len(new))
    return new
  ...
  def recurse(dataset):
    ...
    if R<np.sqrt(N):
      ...
      if half1:
        ...
        _ = recurse(sorted(dataset,key=lambda F:proj(F))[int(R/2):])
    #print( "no of objs ....", model.n_obj)
  recurse(frontier)
  a,b=distant(leaf)
  (good, bad) = (a,b) if cdom(model.solve(a), model.solve(b)) else (b,a)
  #print("Conetenst of good: ", good)
  new=mutate(leaf,good,g=0.5)

  return new

def gale1(iter=1000,pop=1600,model=DTLZ2(n_dec=17, n_obj=2)):
...
def GALE(n_proc=1,frontSize=100,iters=100):
  no_ofobjs = 2    
  t = time()
  collect=[]
  final = []
  ...
  for cc in collect: final.extend(cc)
  ret = gale0(model=DTLZ2(n_dec=17, n_obj=2),new=final,pop=len(final))
  print('Time Taken: ', time()-t)
...
if __name__=="__main__":
  ...
  for item in galeOutput:
    print("######### CHECK ! #########")    
    print("len inside ... ", len(item))        
  print( "Final Out put " , galeOutput)